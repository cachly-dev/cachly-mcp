...
const TOOLS = [
  ...
  // ── New Power Tools ──────────────────────────────────────────────────────────
  {
    name: 'brain_stats',
    description:
      'Get deep statistics about a Brain instance: lesson count, confidence distribution, ' +
      'learning velocity (lessons/day), top topics, top tags, session count, average session length, ' +
      'recall hit rate, and memory crystal count. Use to understand Brain health, measure growth, ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        period_days: {
          type: 'number',
          description: 'Look-back window in days for velocity and session stats (default 30).',
        },
      },
      required: ['instance_id'],
    },
  ...
  {
    name: 'brain_export_md',
    description:
      'Export all lessons from a Brain instance as a Markdown document. ' +
      'Groups lessons by topic, sorted by confidence (highest first). ' +
      'Ideal for onboarding new teammates: share the exported file so their AI ' +
      'starts pre-loaded with your team\'s accumulated knowledge without needing a Brain connection. ' +
      'Optionally filter to a minimum confidence threshold or specific topic prefix. ' +
      'Example: brain_export_md(instance_id="...") · brain_export_md(instance_id="...", min_confidence=0.8, topic_prefix="auth")',
    inputSchema: {
      type: 'object',
      properties: {
        instance_id: { type: 'string', description: 'UUID of the Brain instance.' },
        min_confidence: {
          type: 'number',
          description: 'Only include lessons with confidence ≥ this value (0..1, default 0.5).',
        },
        topic_prefix: {
          type: 'string',
          description: 'Only include lessons whose topic starts with this prefix (e.g. "auth", "deploy"). Omit for all topics.',
        },
        format: {
          type: 'string',
          enum: ['markdown', 'json', 'claude-md'],
          description: '"markdown" (default) — human-readable MD; "json" — structured array; "claude-md" — a CLAUDE.md-compatible block ready to paste.',
        },
      },
      required: ['instance_id'],
    },
  },
  {
    ...
    description:
      'Generate or send a daily digest of Brain activity: lessons learned today, ' +
      'confidence changes, sessions run, top patterns emerging, and predictions for tomorrow. ' +
      'Can be sent to a Telegram chat, a webhook URL, or returned as text. ' +
      'Schedule this in a cron job or CI pipeline to keep the whole team informed about ' +
      'what the Brain learned without anyone having to check manually. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        period_hours: {
          type: 'number',
          description: 'Look-back window in hours (default 24 = last day).',
        },
        webhook_url: {
          type: 'string',
          description: 'POST digest to this URL as JSON (optional). Supports Slack incoming webhooks, Discord webhooks, custom endpoints.',
        },
        telegram_chat_id: {
          type: 'string',
          description: 'Send digest to this Telegram chat ID via cachly\'s Telegram integration (optional, requires Telegram configured on the instance).',
        },
        format: {
          type: 'string',
          enum: ['text', 'markdown', 'slack', 'discord'],
          description: 'Output format. "slack" and "discord" produce webhook-ready Block Kit / Embed payloads.',
        },
      },
      required: ['instance_id'],
    },
  ...
  {
    name: 'brain_onboard_teammate',
    description:
      'Generate a complete onboarding bundle for a new teammate: a shareable link to the Brain, ' +
      'a pre-filled CLAUDE.md / .mcp.json snippet, the top-30 lessons as a quick-start brief, ' +
      'and a setup command they can run in one line. ' +
      'The bundle is returned as a single Markdown document they can paste into any AI assistant ' +
      'or share via Slack/email. ' +
      'Example: brain_onboard_teammate(instance_id="...", name="Sarah")',
    inputSchema: {
      type: 'object',
      properties: {
        instance_id: { type: 'string', description: 'UUID of the Brain instance.' },
        name: {
          type: 'string',
          description: 'New teammate\'s first name (personalises the bundle).',
        },
        editor: {
          type: 'string',
          enum: ['claude-code', 'cursor', 'copilot', 'windsurf', 'cline', 'zed', 'all'],
          description: 'Their primary AI editor (default "all" — includes configs for every editor).',
        },
        top_lessons: {
          type: 'number',
          description: 'How many top lessons to include in the brief (default 30, max 100).',
        },
      },
      required: ['instance_id'],
    },
